...
export const DateRangePicker: FC<DateRangePickerProps> & {
  ...
  const PresetButton: FC<{ preset: string; label: string; isSelected: boolean }> = ({ preset, label, isSelected }) => {
    const handleClick = () => {
      setPreset(preset);
    };
  
    return (
      <Button className={cn(isSelected && "pointer-events-none")} variant="ghost" onClick={handleClick}>
        <>
          <span className={cn("pr-2 opacity-0", isSelected && "opacity-70")}>
            <CheckIcon width={18} height={18} />
          </span>
